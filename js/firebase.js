// Firebase Configuration and Online Multiplayer System
...
async function initOnlineSystem() {
  try {
    ...
    await set(ref(database, 'players/' + onlineState.playerId), {
...
async function findQuickMatch() {
  if (!onlineState.isOnline) return;
  
  const quickMatchRef = ref(database, 'quickMatch');
  const playerData = {
  ...
  if (snapshot.exists()) {
    ...
    if (availableMatch) {
      ...
      await remove(ref(database, `quickMatch/${matchKey}`));
  ...
  setTimeout(() => {
    matchListener();
    remove(ref(database, `quickMatch/${newMatchRef.key}`));
    hideQuickMatchWaiting();
...
function showQuickMatchWaiting() {
  ...
  if (onlineContent) {
    ...
    document.getElementById('cancelQuickMatch').addEventListener('click', () => {
      remove(ref(database, 'quickMatch'));
      hideQuickMatchWaiting();
    });
  }
}
    ...
    if (snapshot.exists()) {
      ...
      Object.keys(matches).forEach(key => {
        if (now - matches[key].timestamp > 30000) { // 30 seconds old
          remove(ref(database, `quickMatch/${key}`));
        }
      });
    }
  });
...
async function createOnlineLobby(opponentId = null, lobbyName = 'Private Lobby', password = '', maxPlayers = 2, isQuickMatch = false) {
  ...
  await set(ref(database, `lobbies/${lobbyCode}`), lobbyData);
...
async function joinLobby(lobbyCode, password = '') {
  ...
  // Add player to lobby
  await update(ref(database, `lobbies/${lobbyCode}/players/${onlineState.playerId}`), {
    name: onlineState.playerName,
    character: gameState.selectedCharacter,
    ready: false
  });
...
async function toggleReadyStatus() {
  ...
  const playerRef = ref(database, `lobbies/${onlineState.currentLobby}/players/${onlineState.playerId}/ready`);
...
async function startOnlineMatch() {
  ...
  await update(ref(database, `lobbies/${onlineState.currentLobby}`), {
    status: 'starting'
  });
  ...
  setTimeout(() => {
    ...
    // The actual match start will be handled in the lobby listener
  }, 1000);
}
...
async function leaveLobby() {
  ...
  await remove(ref(database, `lobbies/${onlineState.currentLobby}/players/${onlineState.playerId}`));
  
  // If host leaves and no players left, delete lobby
  if (onlineState.isHost) {
    const lobbyRef = ref(database, `lobbies/${onlineState.currentLobby}`);
    const snapshot = await get(lobbyRef);
    if (snapshot.exists()) {
      const lobby = snapshot.val();
...
async function addFriend(friendCode) {
  ...
  if (snapshot.exists()) {
    ...
    if (friend) {
      ...
      // Save to database
      await update(ref(database, `players/${onlineState.playerId}/friends`), onlineState.friends);
...
async function inviteFriend(friendId) {
  ...
  await set(ref(database, `invites/${friendId}/${onlineState.playerId}`), inviteData);
  alert('Invite sent!');
}
